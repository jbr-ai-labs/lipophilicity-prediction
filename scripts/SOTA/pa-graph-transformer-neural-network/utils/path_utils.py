...
def get_path_features(rd_mol, path_atoms, path_length, max_path_length,
    ...
    for path_idx in range(len(path_atoms) - 1):
        ...
        # symmetry_bonds.append(int(mol_symmetry_classes[atom_1] == mol_symmetry_classes[atom_2]))

    features = []
    for path_idx in range(max_path_length):
        path_bond = path_bonds[path_idx] if path_idx < len(path_bonds) else None
        # if path_bond is not None:
        #     features.append(np.append(mol_features.get_path_bond_feature(path_bond), symmetry_bonds[path_idx]))
        # else:
    ...
    # print(mol_symmetry_classes, path_atoms)
    if path_atoms:
        features.append(np.array([int(mol_symmetry_classes[path_atoms[0]] == mol_symmetry_classes[path_atoms[-1]])]))
    else:
